...
Nu_h_rho = StabilizationParameter(sol_h.sub(0), sol_n.sub(0), sol_n_.sub(0), u, c_e, c_max, dlfn.Constant(dt), E_rho_norm, time_step)
Nu_h_J = StabilizationParameter(sol_h.sub(1), sol_n.sub(1), sol_n_.sub(1), u, c_e, c_max, dlfn.Constant(dt), E_J_norm, time_step)

# == Problem in variational Form =======
# The fields rho and J are computed as a 'coupled system', although only J depends on rho and rho could is solved
# analytically by a Heaviside-function.
...
F_rec = [F_rho_1 + F_J_1, F_rho_2 + F_J_2, F_rho_3 + F_J_3, F_rho_4 + F_J_4]
F = F_rho + F_J
...
while t_i < t_end:
    ...
    dlfn.solve(lhs(F_rec[0]) == rhs(F_rec[0]), kh[0])
    dlfn.solve(lhs(F_rec[1]) == rhs(F_rec[1]), kh[1])
    dlfn.solve(lhs(F_rec[2]) == rhs(F_rec[2]), kh[2])
    dlfn.solve(lhs(F_rec[3]) == rhs(F_rec[3]), kh[3])
